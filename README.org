...
Once in the Notmuch starting screen you can press =G= to update email. For all the details how Notmuch works please check out the documentation on the project website. It is important to understand, that if you press =G= all it does is to reindex your local mail folder. It does not get the new mail from the server. This is the job of mbsync. To make this smoother we will now create a little scrip as a hook in Notmuch. See [[https://notmuchmail.org/manpages/notmuch-hooks-5/][Notmuch Hooks]] for more info for this. We want to do the following in the terminal:
...
Change to first line to your shell and press =Ctrl+D= or =Ctrl+C= to exit. Now we need to make the file executable by:

#+begin_src sh
chmod +x pre-new
#+end_src

Now when we will press =G= in the Notmuch start screen it will call mbsync to sync all mail from the server(s) and then have Notmuch index it.
