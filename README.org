...
*** Reading Mail
In order to read mail we will use [[https://notmuchmail.org/][Notmuch]]. Have a lool at the projects home page and see the section "Obtaining Notmuch" on how to install it for your operating system.
...
#+begin_src emacs-lisp
  ...
  (setq message-directory "youraccount/Drafts")
...
Since this config is aimed at people having more than one email identity we need a solution for this. We are alredy telling msmtp to choose the right profile according to the from field in our email but in order for the email to be stored in the right sent folder we need to change the ~notmuch-fcc-dirs~ variable accordingly. To solve this we will use [[https://www.emacswiki.org/emacs/gnus-alias.el][gnus-alias]]. Download this and put it in your loading path.
...
If you have more than one email identities you dont need to specify the ~notmuch-fcc-dirs~ as seen above since it will be changed accordingly.
...
As you can see we switch the =Fcc= to specify the correct send folder and also add and remove tags so sent mail will be treated as such.
All the variables in gnus-alias are documented in detail so just inspect them with =C-h v=. For the rules you can do any regular expression to haven more complex filter scenarios to choose the right identity. 
